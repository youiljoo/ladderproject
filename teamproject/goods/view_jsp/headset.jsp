...
<section class="hero-area bg-1 text-center overly::before" style="background-color:white">	
	<!-- Container Start -->
	<div class="container">
		<h2 style="text-align:left; margin-left:-60px; margin-top:-100px;">헤드셋</h2>
		<div class="row">
...
<form method="post" action="${contextPath}/headset/sort.do">	
<ul id="sort">
		<li class="tab_on">
...
<div class="productList_page">
										<ul class="paging">
											<c:if test="${paging.prev }">
												<span><a href='<c:url value="/headset.do?page=${paging.startPage-1 }"/>'>이전</a></span>
											</c:if>
											<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="num">
												<span><a href='<c:url value="/headset.do?page=${num }" />'>${num }</a></span>
											</c:forEach>
											<c:if test="${paging.next && paging.endPage>0 }">
												<span><a href='<c:url value="/headset.do?page=${paging.endPage+1 }"/>'>다음</a></span>
											</c:if>
										</ul>
										</div>
</div>
