...
<form method="post" action="${contextPath}/mouse/sort.do">	
<ul id="sort">
		<li class="tab_on">
...
<div class="productList_page">
										<ul class="paging">
											<c:if test="${paging.prev }">
												<span><a href='<c:url value="/mouse.do?page=${paging.startPage-1 }"/>'>이전</a></span>
											</c:if>
											<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="num">
												<span><a href='<c:url value="/mouse.do?page=${num }" />'>${num }</a></span>
											</c:forEach>
											<c:if test="${paging.next && paging.endPage>0 }">
												<span><a href='<c:url value="/mouse.do?page=${paging.endPage+1 }"/>'>다음</a></span>
											</c:if>
										</ul>
										</div>
</div>
