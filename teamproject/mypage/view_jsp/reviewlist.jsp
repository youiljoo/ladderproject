...
<ul class="my_gnb">
         ...
         <div class="title_wrap"><h2 class="tit_txt">주문현황</h2></div> 
			...
			<c:when test="${empty list}">
  				...
  				<c:otherwise>
								...
								<tbody>
									<tr>
									<td><c:out value="${list.re_num}"/></td>
									<td><c:out value="${list.productName }"/></td>
									<td><a href="${contextPath}/mypage_review_detail.do?re_num=${list.re_num}"><c:out value="${list.re_content}"></c:out></a></td>
		...
		<ul class="paging">
    		<c:if test="${paging.prev}">
        		<span><a href='<c:url value="/mypage_reviewlist.do?page=${paging.startPage-1}"/>'>이전</a></span>
    		</c:if>
    		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="num">
        		<span><a href='<c:url value="/mypage_reviewlist.do?page=${num}"/>'>${num}</a></span>
    		</c:forEach>
    		<c:if test="${paging.next && paging.endPage>0}">
        		<span><a href='<c:url value="/mypage_reviewlist.do?page=${paging.endPage+1}"/>'>다음</a></span>
    		</c:if>
       </ul>
