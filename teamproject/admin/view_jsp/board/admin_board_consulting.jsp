				...
				<c:when test="${empty list}">
     			...
     			<c:forEach items="${list}" var="list">				
								...
								<td align='left' width="15%">
	 							<span><c:out value="${list.c_no}"/></span>                   				
								</td>
								<td align='left' width="40%">
	 							<span></span>
                   				<a class="cls1" href="${contextPath}/admin_board_consulting_detail.do?c_no=${list.c_no}"><c:out value="${list.c_name}"/></a>
								</td>	
								<td align='left'  width="15%">
	  							<span><c:out value="${list.user_id}"/></span>
	  							</td>
	  							<td align='left'  width="15%">
	  							<span><c:out value="${list.c_state}"/></span>
	  							</td>
	...
	<script>
      $(function(){
        $('#searchBtn').click(function() {
          self.location = "admin_board_consulting.do" + '${paging.makeQuery(1)}' + "&searchType=" + $("select option:selected").val() + "&keyword=" + encodeURIComponent($('#keywordInput').val());
        });
      });   
    </script>   
  </div>
	...
	<div class="board_page">	
		<ul class="paging">
    		<c:if test="${paging.prev}">
        		<span><a href="${contextPath}/admin_board_consulting.do?page=${paging.startPage-1}">이전</a></span>
    		</c:if>
    		<c:forEach begin="${paging.startPage}" end="${paging.endPage}" var="num">
    		
        		<span><a href="${contextPath}/admin_board_consulting.do?page=${num}">${num}</a></span>
        		
    		</c:forEach>
    		<c:if test="${paging.next && paging.endPage>0}">
        		<span><a href="${contextPath}/admin_board_consulting.do?page=${paging.endPage+1}">다음</a></span>
    		</c:if>
       </ul>
